...
This document provides a comprehensive hardware compatibility matrix for the PKM-Soft duress protocol, detailing supported System-on-Chips (SoCs), Trusted Execution Environment (TrustZone) implementations, and storage controllers (UFS and eMMC).
...
To ensure the proper execution of cryptographic wiping and duress signal dispatching, storage controllers must meet the following baseline specifications:
...
3. **Replay Protected Memory Block (RPMB):** Mandatory support for hardware-partitioned secure storage to prevent rollback attacks.